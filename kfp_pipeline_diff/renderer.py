...
def generate_mermaid_chart(diff: PipelineDiff) -> str:
    ...
    for name, node in diff.before_nodes.items():
        # Do not include the old renamed node separately
        if name not in all_nodes and name not in diff.renamed_nodes:
            all_nodes[name] = node
    ...
    for name in sorted(all_nodes.keys()):
        ...
        if name in diff.added_nodes:
            ...
            rename_before = None
            for old, new in diff.renamed_nodes.items():
                if new == name:
                    rename_before = old
                    break
            if rename_before:
                label = f"[*] {rename_before} ➜ {name}"
            else:
                label = f"[*] {name}"
            if node.is_subdag:
                label += " (Sub-DAG)"
...
"""
    ...
    parameter_section = ""
    has_param_changes = (
        diff.added_parameters
        or diff.removed_parameters
        or diff.modified_parameters
    )

    if has_param_changes:
        param_rows = []
        # Added parameters
        for p_name in sorted(diff.added_parameters):
            val = diff.after_parameters[p_name]
            p_type = val.get("parameterType", "UNKNOWN") if isinstance(val, dict) else "UNKNOWN"
            p_def = val.get("defaultValue", "_None_") if isinstance(val, dict) else "_None_"
            param_rows.append(f"| `{p_name}` | 🟢 Added | — | `type: {p_type}`, `default: {p_def}` |")
        
        # Removed parameters
        for p_name in sorted(diff.removed_parameters):
            val = diff.before_parameters[p_name]
            p_type = val.get("parameterType", "UNKNOWN") if isinstance(val, dict) else "UNKNOWN"
            p_def = val.get("defaultValue", "_None_") if isinstance(val, dict) else "_None_"
            param_rows.append(f"| `{p_name}` | 🔴 Removed | `type: {p_type}`, `default: {p_def}` | — |")
            
        # Modified parameters
        for p_name, changes in sorted(diff.modified_parameters.items()):
            before_strs = []
            after_strs = []
            for prop, change in sorted(changes.items()):
                before_strs.append(f"`{prop}: {change['before']}`")
                after_strs.append(f"`{prop}: {change['after']}`")
            
            param_rows.append(
                f"| `{p_name}` | 🟡 Modified | {', '.join(before_strs)} | {', '.join(after_strs)} |"
            )
