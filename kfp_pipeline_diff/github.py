...
def post_or_update_sticky_comment(
    ...
    """
    # Enforce GitHub 65KB limit on issue/PR comment bodies
    if len(body) > 65000:
        print("⚠️ Warning: Generated report size is too large for GitHub comment limits (>65KB). Truncating detailed changes...")
        truncate_msg = "\n\n⚠️ **[Report Truncated]** Detailed task property changes were omitted to fit within GitHub's 65KB comment limit. Please run the CLI locally to see the full report.\n"
        
        target_idx = body.find("<summary><b>🔍 Detailed Task Property Changes</b></summary>")
        if target_idx != -1:
            signature = "\n---\n_Report generated by **kfp-pipeline-diff**_"
            body = body[:target_idx + len("<summary><b>🔍 Detailed Task Property Changes</b></summary>")] + "\n\n... (detailed changes truncated to fit size limits) ...\n" + truncate_msg + signature
        else:
